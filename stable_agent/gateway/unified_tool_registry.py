...
class UnifiedToolRegistry:
    ...
    def _h_task_os_agent(self, ctx: RunContext, args: dict[str, Any]) -> StableAgentToolResult:
        ...
        V12.0: 委托给 OSAgentHandler (薄编排层)。
        OSAgentHandler 内部编排 Executor → Curator → ContractBuilder。
        """
        ...
        try:
            ...
            if not hasattr(self, '_os_agent_handler') or self._os_agent_handler is None:
                self._os_agent_handler = OSAgentHandler(
                    orchestrator=self._orchestrator,
                    tool_router=self._tool_router,
                )

            result = self._os_agent_handler.handle(ctx, args)
            ...
            return self._make_result(
                ...
                plain_text_zh=result["plain_text_zh"],
                plain_text_en=result["plain_text_en"],
                dashboard_url=result["dashboard_url"],
                is_error=result["is_error"],
            )
        except Exception as exc:
            logging.getLogger(__name__).exception("OSAgentHandler 执行失败: %s", exc)
            return self._make_result(
                ctx, tool_name,
                ok=False, is_error=True,
                ...
                dashboard_url=f"/runs/{ctx.run_id}" if args.get("open_dashboard", True) else "",
            )

    # V12.0: 原始内联实现已删除，全部委托给 OSAgentHandler
    # (见上方 _h_task_os_agent)
